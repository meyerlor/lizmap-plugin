...
class ProjectManager(LizmapProtocol):
    ...
    def project_config_file(
        ...
        if lwc_version >= LwcVersions.Lizmap_3_6:
            ...
            logger.info(
                f"Project has been detected : {'VALID' if valid else 'NOT valid'} "
                "according to OGC validation."
            )
        ...
        if is_lizmap_cloud(server_metadata):
            ...
            if self.dlg.current_server_info(ServerComboData.LwcBranchStatus.value) in eol:
                if (
                    self.dlg.current_server_info(ServerComboData.LwcBranchStatus.value)
                    ...
                    msg = tr(
                        ...
                        "It is in <strong>security bugfix mode only</strong>, it means only critical "
                        "bugfix are fixed and soon the branch will be declared "
                ...
                QMessageBox.warning(
                    ...
                    + "<br><br>"
                    + tr(
                        "You might have some old project which need an update from you. "
                        "The list is written on the dashboard. Projects are not deleted "
                        "during the update of Lizmap Web Client, they will be only invisible "
                        "on the main landing page until they are updated by you."
                    )
                    + "<br><br>"
            ...
            if manager:
                try:
                    ...
                    panel_name = self.dlg.mOptionsListWidget.item(
                        self.layers_table[key].get("panel", key)
                    ).text()
                    QMessageBox.critical(
                        self.dlg,
                        "Lizmap",
                        tr(
                            'An error has been raised while saving table "<strong>{name}</strong>", '
                            "maybe it was due to invalid layers ?"
                        ).format(name=panel_name)
                        + "\n\n"
                        + tr(
                            "If yes, saving the Lizmap configuration file with invalid layers "
                            "in the legend is not currently supported by the plugin."
                        )
                        + "\n\n"
                        + tr(
                            'Please visit the corresponding tab "<strong>{name}</strong>" '
                            "and check if you have a warning in one of the column."
                        ).format(name=panel_name)
                        + "\n\n"
        ...
        if not isinstance(self.layerList, dict):
            ...
            QMessageBox.warning(
                ...
                "Lizmap",
                tr(
                    '"Apply" or "OK" are not supported right now. Please close the dialog and '
                    "reopen the plugin."
        ...
        for k, v in self.layerList.items():
                    ...
                    if key == "noLegendImage":
                        ...
                        if v["legend_image_option"] == "expand_at_startup" and with_gui:
                            ...
                            QMessageBox.warning(
                                ...
                                tr("Legend image"),
                                tr(
                                    'Be careful, the option "Expand at startup" for the layer '
                                    '"{layer_name}" is not '
            ...
            if (
                meta_tile_size is not None
                and isinstance(meta_tile_size, str)
                and not re.match(r"\d,\d", meta_tile_size)
            ):
            ...
            if (
                ambiguous_to_bool(layer_options["popup"])
                and layer_options["popupSource"] == "lizmap"
    ...
    def check_project(
        ...
        logger.info(
            f"Project has been detected : {'VALID' if valid else 'NOT valid'} according to OGC validation."
        )
        if not valid:
        ...
        if lwc_version >= LwcVersions.Lizmap_3_6:
            ...
            if as_boolean(use_layer_id):
                self.dlg.check_results.add_error(
                    Error(Path(self.project.fileName()).name, checks.WmsUseLayerIds)
                )
        ...
        if lwc_version >= LwcVersions.Lizmap_3_7 and with_gui:
            ...
            if (
                ...
                and crs_inverted
                and is_not_4326
                and json_meta["info"]["version"] in impacted_versions
            ):
                # https://github.com/3liz/lizmap-web-client/issues/4191
                self.dlg.check_results.add_error(
                    Error(Path(self.project.fileName()).name, checks.CrsInvertedAxis)
                )
            ...
            if results:
                ...
                self.dlg.log_panel.append("<br>")
                self.dlg.log_panel.append(
                    "<em>" + tr("A leading or a trailing spaces can be added.") + "</em>"
                )
                self.dlg.log_panel.start_table()
                self.dlg.log_panel.append(
        ...
        if check_server:
            ...
            if results:
                if beginner_mode:
                    self.dlg.log_panel.append(
                        tr(
                            "The process is stopping, the Lizmap configuration file is not going "
                            "to be generated because some safeguards are not compatible and you are "
                            "using the 'Beginner' mode. Either fix these "
                    ...
                    self.dlg.log_panel.append(
                        tr(
                            "The process is continuing but these layers might be invisible "
                            "if the server is not well configured or if the project is not "
                            "correctly uploaded to the server."
                        ),
                        Html.P,
                    )
        ...
        if check_server:
            ...
            # These checks are only done if we find the server configuration below,
            # starting with version 2.9.4
            ...
            #
            #                 uri_filter = '<ul>' + ''.join(
            #                     [f"<li>{k}</li>" for k in filters.keys()]
            #                 ) + '</ul>'
            #
            #                 self.dlg.log_panel.append(uri_filter, Html.Td)
            #
            #                 layer_names = '<ul>' + ''.join(
            #                     [f"<li>{k}</li>" for k in filters.values()]
            #                 ) + '</ul>'
            #
        ...
        if self.project.readNumEntry("WMSMaxAtlasFeatures", "")[0] <= 0:
            logger.info(
                "The maximum atlas features was less than '1'. We set it to '1' to at least have a value."
            )
        ...
        if self.dlg.check_results.has_blocking():
            msg = tr(
                'You are using the "{mode}" mode and you have some <strong>blocking</strong> checks.'
            ).format(mode=mode)
        else:
        ...
        if important_issues >= 1 and lizmap_cloud:
            ...
            self.dlg.log_panel.append(
                tr(
                    "{count} error(s) have been found on the project, user has agreed with "
                    "these errors : {result}"
                ).format(
                    count=important_issues, result="<strong>" + tr("Yes") + "<strong>" if result else tr("No")
                ),
                Html.P,
            )
            if not result:
    ...
    def clean_project(self):
        ...
        if layers:
            message = tr(
                ...
                "They have been removed. You must save your project."
            ).format(", ".join(layers))
    ...
    def check_global_project_options(self) -> Tuple[bool, str]:
        ...
        if self.project.baseName() != unaccent(self.project.baseName()):
            message = tr(
                "Your file name has some accents in its name. "
                "The project file name mustn't have accents in its name."
            )
            return False, message + base_message

        message = tr(
            "You mustn't open the QGS file located in your local webdav directory. "
            "Please open a local copy of the "
            "project."
        )
        # Windows
        network_dav = []
        ...
        # Linux :
        # /run/user/1000/gvfs/dav:host=...,ssl=true,....,prefix=%2Flizmap_3_6%2Fdav.php \
        # /...tests_projects/..qgs
        ...
        if settings.value("Qgis/capitaliseLayerName") and settings.value(
            "Qgis/capitaliseLayerName", type=bool
        ):
            message = tr(
        ...
        if not project_wms_extent:
            ...
            if (
                not project_wms_extent[0]
                or not project_wms_extent[1]
                or not project_wms_extent[2]
                or not project_wms_extent[3]
            ):
